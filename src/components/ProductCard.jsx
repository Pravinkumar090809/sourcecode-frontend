  ...
  return (
    <Link href={`/products/${product.id}`} className="group block animate-fadeIn" style={{ animationDelay: `${index * 80}ms` }}>
      <div className="glass rounded-2xl overflow-hidden transition-all duration-300 hover:border-red-500/30 hover:shadow-lg hover:shadow-red-500/5 hover:-translate-y-1">
        ...
        <div className="relative h-48 overflow-hidden" style={{ background: "linear-gradient(135deg, rgba(239,68,68,0.08), rgba(139,92,246,0.08))" }}>
          {thumbnail && !imgError ? (
            <img
              src={thumbnail}
          ...
          {/* Overlay gradient for readability when thumbnail is present */}
          {thumbnail && !imgError && (
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
          )}
          {/* Price badge */}
          <div className="absolute top-3 right-3 z-10">
            <span className="px-2.5 py-1 rounded-full text-xs font-bold text-white shadow-lg" style={{ background: "linear-gradient(135deg,#ef4444,#dc2626)" }}>
              ₹{price}
            </span>
          </div>
          {/* Rating badge */}
          <div className="absolute top-3 left-3 z-10 flex items-center gap-1 px-2 py-1 rounded-full text-xs" style={{ background: "rgba(0,0,0,0.6)", backdropFilter: "blur(10px)" }}>
            <HiOutlineStar className="text-yellow-400 text-xs" />
            <span className="text-white font-medium">4.8</span>
          </div>
          {/* Instant Download badge at bottom of image */}
          <div className="absolute bottom-3 left-3 z-10 flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-medium text-emerald-300" style={{ background: "rgba(0,0,0,0.6)", backdropFilter: "blur(10px)" }}>
            <FiDownload className="text-xs" /> Instant Download
          </div>
        </div>
        ...
        <div className="p-4">
          ...
          <p className="text-slate-500 text-xs mb-3 line-clamp-2 leading-relaxed">
            {product.description ? product.description : (
              <span className="italic text-red-300">[No description]</span>
            )}
          </p>
          {tags.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mb-3">
              ...
              {tags.length > 3 && (
                <span className="px-2 py-0.5 rounded-md text-[10px] text-slate-500" style={{ background: "rgba(255,255,255,0.03)" }}>
                  +{tags.length - 3}
                </span>
              )}
            </div>
          ...
          <div className="flex items-center justify-between pt-3 border-t border-white/5">
            <div className="flex items-center gap-1.5">
              <span className="text-[10px] font-medium text-red-400/70">₹{price}</span>
              {price > 99 && (
                <span className="text-[10px] text-slate-600 line-through">₹{price * 2}</span>
              )}
            </div>
